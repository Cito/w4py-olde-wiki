...
def str2html(sourcestring, colors=None, form=None):
    ...
    Parser(sourcestring, colors, out=stringIO).format(form)
    stringIO.seek(0)
...
def path2html(sourcepath, colors=None, form=None):
    ...
    stringIO = cStringIO.StringIO()
    sourcestring = open(sourcepath).read()
    Parser(sourcestring, colors, title=sourcepath, out=stringIO).format(form)
    stringIO.seek(0)
    return stringIO.read()
...
class Parser:
    ...
    def _getStyle(self, key):
        ...
        rawcolor = self.colors.get(key, self.colors[NAME])
        return rawcolor
...
content="text/html;charset=iso-8859-1">\n')
    ...
    def _sendCSSText(self, toktype, toktext):
        ...
        if color == '#000000' and not tags:
            ...
            startspan = '<span class="%s">'%(self._getMarkupClass(toktype))
            endspan = '</span>'
        sendtext = cgi.escape(toktext)
